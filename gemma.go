...
//go:generate go run github.com/gx-org/gx/golang/packager --gx_package_module=github.com/gx-org/gemma/gemma
//go:generate go run github.com/gx-org/gx/golang/binder/genbind --gx_package=github.com/gx-org/gemma/gemma
...
func (g *Gemma) Prompt(prompt string) (string, error) {
	...
	state, _, tokenHandle, err := g.network.NewSamplingState().Run(promptEncoded)
	if err != nil {
	...
	for i := promptSize; i < g.params.NumSamplingSteps; i++ {
		newState, _, tokenHandle, err := g.network.SampleStep().Run(state)
		if err != nil {
